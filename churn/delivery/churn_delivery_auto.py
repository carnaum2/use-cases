...
def set_paths_and_logger():
    ...
    if pathname.startswith("/var/SP/data/bdpmdses/deliveries_churn/"):
        ...
        # Add deliveries_churn into path to be able to load 'common' directory
        if "/var/SP/data/bdpmdses/deliveries_churn/" not in sys.path:
            sys.path.append("/var/SP/data/bdpmdses/deliveries_churn/")
            print("Added '{}' to path".format("/var/SP/data/bdpmdses/deliveries_churn/"))
    ...
    logging_file = os.path.join("/var/SP/data/bdpmdses/deliveries_churn/logs_delivery", "churn_delivery_auto_" + dt.datetime.now().strftime("%Y%m%d_%H%M%S") + ".log")
...
if __name__ == "__main__":
    ...
    from churn.delivery.delivery_constants import MODEL_EVALUATION_NB_DAYS, DELIVERY_TABLE_PREPARED, ALL_SEGMENTS
    ...
    parser.add_argument('-c', '--closing_day', metavar='<YYYYMMDD>', type=str, required=False,
    ...
    arg_closing_day = args.closing_day
    arg_historic = args.historic

    print("detected parse args closing_day={} historic={}".format(arg_closing_day, arg_historic))
    ...
    if logger: logger.info("Computing automatically the arguments....")
    from pykhaos.utils.date_functions import move_date_n_days, get_previous_cycle, move_date_n_cycles
    import datetime as dt
    today = dt.datetime.today().strftime("%Y%m%d")

    if arg_closing_day:
        print("Using introduced arg_closing_day {}".format(arg_closing_day))
        closing_day = arg_closing_day

        tr_cycle_ini = move_date_n_cycles(closing_day, n=-8)
        tr_cycle_end = tr_cycle_ini
        print("arg closing_day={} {} | closing_day={} training_day={}".format(closing_day,
                                                                              dt.datetime.strptime(closing_day, "%Y%m%d").strftime('%A')[:3],
                                                                              closing_day,
                                                                              tr_cycle_ini))

    else:
        closing_day = get_previous_cycle(move_date_n_days(today, n=-3))

        tr_cycle_ini = get_previous_cycle(move_date_n_cycles(move_date_n_days(today, n=-4), n=-8))
        tr_cycle_end = tr_cycle_ini
        print("today={} {} | closing_day={} training_day={}".format(today, dt.datetime.strptime(today, "%Y%m%d").strftime('%A')[:3], closing_day, tr_cycle_ini))
    ...
    historic = arg_historic
    horizon = 8
    deliv_file_format = "reasons"
    scores_segments = "all"
    ref_date = None
    ...
    if logger: logger.info("")
    if logger: logger.info("- - - - - - - - - - - - - - - - - - - - - - - - ")
    if logger: logger.info("AUTOMATIC PARAMS:")
    ...
    spark = (SparkSession.builder.appName("churn_delivery_{}".format(closing_day))
             .master("yarn")
    ...
    if check or delivery:
        ...
        checker_scores_fbb_obj = checker(spark, partial(check_scores_new, spark, closing_day, segment="fbb",
                                                            tr_cycle_ini=tr_cycle_ini,
                                                            tr_cycle_end=tr_cycle_end), partial(format_table_msg),
                                             closing_day, SECTION_SCORES_NEW + " " + "fbb")
        ...
        checkers_list = [checker_car_obj, checker_join_prev_car_obj, checker_extra_feats_obj, checker_prepared_car_obj,
                         checker_scores_fbb_obj, checker_scores_onlymob_obj, checker_scores_mobileandfbb_obj]
    ...
    if delivery:
        ...
        for segmentfilter in scores_segments_list:
            ...
            elif segmentfilter == "fbb":
                checker_scores_obj = checker_scores_fbb_obj
            else:
            ...
            if not checker_scores_obj.status: # incomplete step
                ...
                if segmentfilter != "fbb":
                    ...
                    if not checker_scores_fbb_obj.status:
                        # FBB MODEL
                        if logger: logger.info("About to launch scores for model fbb - '{}'".format(CMD_SCORES_FBB.format(date=closing_day)))
                        popen_list.append(launch_process(CMD_SCORES_FBB.format(date=closing_day)))
            ...
            # launch in parallel the churn reasons corresponding to this segment
            # .   .   .   .   .   .
            # CHURN REASONS
            # churnreasons for fbb is launched later, to be sure fbb model is finished
            if not checker_churnreasons_table_obj.status and segmentfilter != "fbb":
                print("SegmentFilter={} status={}".format(segmentfilter, checker_churnreasons_table_obj.status))
                print(checker_churnreasons_table_obj.extra_info[segmentfilter])
        ...
        while any_running(popen_list):
            ...
            checker_churnreasons_table_obj.print_msg() # At this point, churn reasons for fbb is not launched
            checker_scores_fbb_obj.check_func()
            checker_scores_fbb_obj.print_msg()
            if logger: logger.info("\n-------------------\n")



        print("SUMMARY CHURN REASONS")
        print(checker_churnreasons_table_obj.extra_info)

        # if clause just in case the script is relaunched
        if checker_churnreasons_table_obj.extra_info["fbb"]:
            # Launch fbb churn reasons since fbb scores are not finished yet
            fbb_cmd_process = CMD_CHURN_REASONS_FBB.format(dir=root_dir if root_dir.endswith("/") else root_dir+"/", date=closing_day, pred_name=build_pred_name(spark, closing_day, "fbb", horizon))
            if logger: logger.info("About to launch churn reasons for segment fbb - '{}'".format(fbb_cmd_process))
            churn_reasons_fbb_popen = launch_process(fbb_cmd_process)

            while any_running([churn_reasons_fbb_popen]):
                if logger: logger.info("Not all process open from Popen are finished yet. Current state:")
                print_threads_status([churn_reasons_fbb_popen])
                time.sleep(5 * 60)  # wait 5 minutes
                checker_churnreasons_table_obj.check_func()
            ...
            print("churn reasons for fbb does not require to be launched")
    ...
    if not checker_model_evaluation_obj.status:
