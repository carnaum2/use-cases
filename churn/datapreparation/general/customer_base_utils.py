...
# Copied into use-cases/churn_nrt/src/data/customer_data.py
...
# Copied into use-cases/churn_nrt/src/data/customer_data.py as is_null_date
...
# Copied to use-cases/churn_nrt/src/data/customer_data.py
...
def get_services(spark, date_):
    ...
    ClosingDay_date = datetime.date(int(closing_day[:4]), int(closing_day[4:6]), int(closing_day[6:8]))
    yesterday = ClosingDay_date + datetime.timedelta(days=-1)
...
# Copied to use-cases/churn_nrt/src/data/customer_base.py
def get_customer_base(spark, date_, cod = "99"):
    ...
    .filter((col('rgu') != 'prepaid') & (col("clase_cli_cod_clase_cliente") == "RS") & (col("cod_estado_general").isin("01", "09", cod)) & (col("srv_basic").isin("MRSUI", "MPSUI") == False))\
    .select('NUM_CLIENTE', 'NIF_CLIENTE', 'msisdn', 'rgu', 'campo2')\
    ...
    base_df = base_df.withColumnRenamed("campo2", "msisdn_d")
    return base_df

# use-cases/churn_nrt/src/data/customer_base.py
def get_customer_base_segment(spark, date_, cod = "99"):

    VALUES_CUST_LEVEL_AGG = ['fbb', 'mobile', 'tv', 'prepaid', 'bam_mobile', 'fixed', 'bam']

    raw_base_df = get_customer_base(spark, date_, cod)
    ...
    segment_base_df = raw_base_df.join(data_CAR_CUST_tmp2.select('NIF_CLIENTE', 'seg_pospaid_nif'), ['NIF_CLIENTE'], 'inner')
