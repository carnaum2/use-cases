...
TABLE_NIF = '/data/udf/vf_es/churn/churn_reasons/churn_reasons_NIF_MOB'
...
if __name__ == "__main__":
    ...
    spark = (SparkSession.builder.appName("churn_reasons_improved")
             .master("yarn")
    ...
    #Feature mapper        
        use_var, bill_var, inter_var, spinners_var, serv_var,  penal_var = getFeatGroups(spark, df_car, feat_list[:200], n_feats, 1)
    ...
    .withColumn('executed_at', lit(executed_at)).withColumn('segment',lit(args.segment)).withColumn('training_closing_date', lit(pred_name)).withColumn('closing_day', lit(str(args.closing_d))) 
    save_df.repartition(300).write.save(save_dir, format='parquet', mode='append')
    
    save_df2 = df_final.select(['msisdn', 'num_cliente', 'nif_cliente'] + score_cols_2).withColumn('service', lit('mobile')).withColumn('training_closing_date', lit(pred_name)).withColumn('closing_day', lit(str(args.closing_d)))
    save_df2.repartition(300).write.save(save_dir_nif, format='parquet', mode='append')
    
    #df_final.select(['msisdn'] +  name_cols + score_cols_2 + score_cols +['Incertidumbre']).write.format('parquet').mode('append').saveAsTable(save_dir)
