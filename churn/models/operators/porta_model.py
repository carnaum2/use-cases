...
if __name__ == "__main__":
    ...
    df_no_etiquetado_train=get_ids_filtered('20190531',spark)

    print('[Info]: Getting IDS for test set...')

    df_no_etiquetado_test=get_ids_filtered('20190731',spark)

    print('[Info]: Training and test set without labels obtained')

    print('[Info]: Labelling training set...')

    portab_final_train = get_portab_next_month('20190531', spark)

    portab_final_test = get_portab_next_month('20190731', spark)

    print('[Info]: Labelling training set...')

    train_final=get_portab_labels(portab_final_train, df_no_etiquetado_train)

    test_final = get_portab_labels(portab_final_test, df_no_etiquetado_test)

    print('[Info]: Operators volume for training set...')

    #train_final.groupby('Operador_target').count().show()

    print('[Info]: Operators volume for test set...')

    #test_final.groupby('Operador_target').count().show()

    #Get important variables to include in each model
 ...
 'otros']
    ...
    test_final_masmovil=test_final.select([c for c in train_final.columns if c in var_masmovil])
    test_final_movistar=test_final.select([c for c in train_final.columns if c in var_movistar])
    test_final_orange=test_final.select([c for c in train_final.columns if c in var_orange])
    test_final_otros=test_final.select([c for c in train_final.columns if c in var_otros])
    ...
    train_unbal_masmovil, train_masmovil, validation_masmovil, test_masmovil = preparation('masmovil', train_final_masmovil,
    ...
    print('[Info]: AUC masmovil train', auc_masmovil_train)
    auc_masmovil_validation = evaluator.evaluate(preds_masmovil_validation)
    print('[Info]: AUC masmovil validation', auc_masmovil_validation)
    auc_masmovil_test = evaluator.evaluate(preds_masmovil_test)
    print('[Info]: AUC masmovil test', auc_masmovil_test)
    ...
    print('[Info]: AUC movistar train', auc_movistar_train)
    auc_movistar_validation = evaluator.evaluate(preds_movistar_validation)
    print('[Info]: AUC movistar validation', auc_movistar_validation)
    auc_movistar_test = evaluator.evaluate(preds_movistar_test)
    print('[Info]: AUC movistar test', auc_movistar_test)
    ...
    print('[Info]: AUC orange train', auc_orange_train)
    auc_orange_validation = evaluator.evaluate(preds_orange_validation)
    print('[Info]: AUC orange validation', auc_orange_validation)
    auc_orange_test = evaluator.evaluate(preds_orange_test)
    print('[Info]: AUC orange test', auc_orange_test)
    ...
    print( '[Info]: AUC otros train', auc_otros_train)
    auc_otros_validation = evaluator.evaluate(preds_otros_validation)
    print( '[Info]: AUC otros validation', auc_otros_validation)
    auc_otros_test= evaluator.evaluate(preds_otros_test)
    print( '[Info]: AUC otros test', auc_otros_test)

    preds_masmovil_test = preds_masmovil_test.select('msisdn', 'score', 'prediction', 'masmovil')
    preds_movistar_test = preds_movistar_test.select('msisdn', 'score', 'prediction', 'movistar')
    preds_orange_test = preds_orange_test.select('msisdn', 'score', 'prediction', 'orange')
    preds_otros_test=preds_otros_test.select('msisdn','score','prediction','otros')
    ...
    predicciones_union_norm=predicciones_union_norm.withColumn('calib_score_otros',col('calib_score_otros')/(col('calib_score_masmovil')+
                                                                                                                             col('calib_score_movistar')+
                                                                                                                          col('calib_score_orange')+
                                                                                                                          col('calib_score_otros')))
    ...
    prediccionesUnion=predicciones_union_norm.withColumn('Operador_real',
                    when(predicciones_union_norm['masmovil']==1,1).otherwise(
                        when(predicciones_union_norm['movistar']==1,2).otherwise(
                            when(predicciones_union_norm['orange']==1,3).otherwise(
                                when(predicciones_union_norm['otros']==1,4)))))

    prediccionesUnion=prediccionesUnion.drop('masmovil','movistar','orange','otros')
    ...
    prediccionesFinal=prediccionesFinal.withColumn('Operador_predicho',prediccionesFinal['Operador_predicho'].cast(DoubleType()))
    prediccionesFinal=prediccionesFinal.withColumn('Operador_real',prediccionesFinal['Operador_real'].cast(DoubleType()))

    prediccionesFinal=prediccionesFinal.orderBy('Prob_max',ascending=False)

    print('[Info]: Final predictions dataframe created')

    prediccionesFinal.repartition(300).write.save('/data/udf/vf_es/churn/portabPropension_model/prediccionesFinal',
                                              format='parquet', mode='overwrite')

    print('[Info]: Final predictions dataframe saved')


    #predicciones_top=prediccionesFinal.limit(50000).select('msisdn') añadir el flag de que si se encuentra en el top 5000 de mi modelol es certera
