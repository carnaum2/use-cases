...
if __name__ == "__main__":
    ...
    print('Balancing tr df for model with wrong predictions')
    bad_b = balance_df2(bad, 'label')
    print('Balancing tr df for model with correct predictions')
    ...
    good_p = good_b.drop(col('probability')).drop(col('prediction')).drop(col('rawPrediction')).drop(col('model_score'))
    bad_p = bad_b.drop(col('probability')).drop(col('prediction')).drop(col('rawPrediction')).drop(col('model_score'))

    model_good = pipeline2.fit(good_p)
    model_bad  = pipeline2.fit(bad_p)
    ...
    tr_good = model_good.transform(good_p).drop(col('features'))
    tr_bad = model_bad.transform(bad_p).drop(col('features'))

    tr_preds_good = tr_good.withColumn("model_score", getScore(col("probability")).cast(DoubleType()))
    tr_calib_preds_good = calibmodel_good[0].transform(tr_preds_good).withColumnRenamed('calib_model_score', "model_score_good_calib")
    tr_preds_bad = tr_bad.withColumn("model_score", getScore(col("probability")).cast(DoubleType()))
    tr_calib_preds_bad = calibmodel_bad[0].transform(tr_preds_bad).withColumnRenamed('calib_model_score', "model_score_bad_calib")

    final_feats = ["model_score_good_calib", "model_score_bad_calib"] #+ featCols

    assembler3 = VectorAssembler(inputCols=final_feats + featCols, outputCol="features")
    ...
    trdf_2 = model_good.transform(trdf).drop(col('features'))

    trdf_wg = trdf_2.withColumn("model_score", getScore(col("probability")).cast(DoubleType()))
    trdf_wg_calib = calibmodel_good[0].transform(trdf_wg).withColumnRenamed('model_score', "model_score_good_calib")\
    .drop(col("calib_model_score")).drop(col("probability")).drop(col("features")).drop(col('prediction')).drop(col('rawPrediction'))#\
    #.drop(col('calib_model_score'))

    trdf_wb_ = model_bad.transform(trdf_wg_calib).drop(col('features'))

    trdf_wb = trdf_wb_.withColumn("model_score", getScore(col("probability")).cast(DoubleType()))
    trdf_wb_calib = calibmodel_bad[0].transform(trdf_wb).withColumnRenamed('model_score', "model_score_bad_calib")\
    .drop(col("calib_model_score")).drop(col("probability")).drop(col("features")).drop(col('prediction')).drop(col('rawPrediction'))#\
    #.drop(col('calib_model_score'))

    model_f = pipeline3.fit(trdf_wb_calib)
    valdf_2 = model_good.transform(valdf).drop(col('features'))

    valdf_wg = valdf_2.withColumn("model_score", getScore(col("probability")).cast(DoubleType()))
    valdf_wg_calib = calibmodel_good[0].transform(valdf_wg).withColumnRenamed('model_score', "model_score_good_calib")\
    .drop(col("calib_model_score")).drop(col("probability")).drop(col("features")).drop(col('prediction')).drop(col('rawPrediction'))

    valdf_wb_ = model_bad.transform(valdf_wg_calib).drop(col('features'))

    valdf_wb = valdf_wb_.withColumn("model_score", getScore(col("probability")).cast(DoubleType()))
    valdf_wb_calib = calibmodel_bad[0].transform(valdf_wb).withColumnRenamed('model_score', "model_score_bad_calib")\
    .drop(col("calib_model_score")).drop(col("probability")).drop(col("features")).drop(col('prediction')).drop(col('rawPrediction'))
    
    calibmodel_final = get_calibration_function2(spark, model_f, valdf_wb_calib, 'label', 10)
    #calibmodel_final = get_calibration_function2(spark, model_f, valdf, 'label', 10)

    final_tr_df_with_scores = model_f.transform(trdf_wb_calib).withColumn("model_score", getScore(col("probability")).cast(DoubleType()))

    final_tr_df_with_calib_scores = calibmodel_final[0].transform(final_tr_df_with_scores).withColumnRenamed('calib_model_score', "final_score_calib")
    ...
    ttdf_2 = model_good.transform(ttdf).drop(col('features'))

    ttdf_wg = ttdf_2.withColumn("model_score", getScore(col("probability")).cast(DoubleType()))
    ttdf_wg_calib = calibmodel_good[0].transform(ttdf_wg).withColumnRenamed('calib_model_score', "model_score_good_calib")\
    .drop(col("model_score")).drop(col("probability")).drop(col("features")).drop(col('prediction')).drop(col('rawPrediction'))#\
    #.drop(col('calib_model_score'))

    ttdf_wb_ = model_bad.transform(ttdf_wg_calib).drop(col('features'))

    ttdf_wb = ttdf_wb_.withColumn("model_score", getScore(col("probability")).cast(DoubleType()))
    ttdf_wb_calib = calibmodel_bad[0].transform(ttdf_wb).withColumnRenamed('calib_model_score', "model_score_bad_calib")\
    .drop(col("model_score")).drop(col("probability")).drop(col("features")).drop(col('prediction')).drop(col('rawPrediction'))#\
    #.drop(col('calib_model_score'))

    final_tt_df_with_scores = model_f.transform(ttdf_wb_calib).withColumn("model_score",
                                                                          getScore(col("probability")).cast(
                                                                   DoubleType()))
    final_tt_df_with_calib_scores = calibmodel_final[0].transform(final_tt_df_with_scores).withColumnRenamed(
        'calib_model_score', "final_score_calib")
    tt_PredictionAndLabels = final_tt_df_with_calib_scores.select(['final_score_calib', 'label']).rdd.map(
        lambda r: (r['final_score_calib'], r['label']))
    ...
    #Entrenar el ultimo modelo DONE
    #Mucho ojo a que nombres espera la funcion de calibracion OKAY
