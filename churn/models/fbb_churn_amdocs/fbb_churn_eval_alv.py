...
if __name__ == "__main__":
    ...
    #ath = '/data/udf/vf_es/churn/fbb_tmp/inittrdf_ini_' + tr_ttdates
    #if pathExist(path):
    ...
    save_dir = 'tests_es.asaezco_fbb_improved_10_' + args.prediction_day
    ...
    ensemble = model_f.transform(df_aux).withColumn("model_score", getScore(col("probability")).cast(DoubleType()))

    ttPredictionAndLabels = ensemble.select(['model_score_', 'label']).rdd.map(
        lambda r: (r['model_score'], r['label']))
    ttmetrics = BinaryClassificationMetrics(ttPredictionAndLabels)
    print'Ensemble AUC: {}'.format(ttmetrics.areaUnderROC)

#\
        #.withColumn('medio', mean(*cols_scores))  # .withColumn('minimo', least(*cols_scores))
    ...
    #print "[Info Main FbbChurn] " + time.ctime() + " Deleting temporary files "

    #subprocess.call(["hadoop", "fs", "-rm", "-r", '/data/udf/vf_es/churn/fbb_tmp/inittrdf_' + tr_ttdates])
    #subprocess.call(["hadoop", "fs", "-rm", "-r", '/data/udf/vf_es/churn/fbb_tmp/inittrdf_tmp_' + tr_ttdates])
