...
def getProcessedCrmColumns():
    crm = getInitialCrmColumns() + getNewCrmColumns()
    return(Diff(crm, [
    "fecha_migracion",
    "bam_fx_first",
    "bam-movil_fx_first",
...
def getRawMobileColumns(): 
    return(getIdFeats()+(getInitialCrmColumns())+(getInitialGnvColumns())+(getInitialCampColumns())+(getInitialBillingColumns())+(getInitialNumclienteColumns()).distinct())

def getProcessedColumns(): 
    return(getIdFeats()+(getProcessedCrmColumns())+(getProcessedGnvColumns())+union(getProcessedCampColumns())+(getProcessedBillingColumns())+(getProcessedNumclienteColumns()))

def getFeatSubset(feats2rem):
    removed_feats = getFeats2RemFromList(feats2rem)
    return(Diff(getProcessedColumns(),removed_feats))
