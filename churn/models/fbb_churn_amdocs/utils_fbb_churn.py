...
def getFbbChurnLabeledCarCycles(spark, origin, yearmonthday, selcols, horizon = 4):
    ...
    unbaltrdf = trfeatdf\
    .join(fixporttr, ['msisdn'], "left_outer")\
    ...
    ppp= trfeatdf.join(fixporttr, ['msisdn'], "inner")
    print('********* Uniooooon',ppp.count())

    print("[Info getFbbChurnLabeledCar] " + time.ctime() + " Portability Requests for month " + yearmonth + ": " + str(fixporttr.count()))
    print("[Info getFbbChurnLabeledCar] " + time.ctime() + " Losses for month " + yearmonth + ": " + str(fixdxtr.count()))
...
def get_orders_df(spark, origin, refdate):
    ...
    ref_yearmonth = move_date_n_yearmonths(refdate[:6], n=-1)  
    newrefdate = get_closure(ref_yearmonth)
...
def saveResults(tr_calib_preds_df, df_scores, trmetrics, ttmetrics, feat_importance, trcycle_ini, horizon, ttcycle_ini,spark,elapsed_time,nowMoment):
    ...
    executedAt_str = nowMoment.strftime('%Y-%m-%d %H:%M:%S')
    executedAt_time = str(nowMoment.hour) + str(nowMoment.minute) + str(nowMoment.second)
    date_name=str(nowMoment.year)+str(nowMoment.month)+str(nowMoment.day)
    ...
    description='Model to identify the fibre clients with risk of leaving the company.'
    target='Fiber clients with portability request or that have cancelled their fiber line.'
    trcycle_fin=trcycle_ini

    # training_closing_date should be the date between trcycle_ini and trcycle_end. At this moment, in the code ttcycle_ini=trcycle_end

    modelParam = [{'model_name': 'churn_preds_fbb',
                   'executed_at': executedAt_str,
                   'model_level': 'client_id',
                   'training_closing_date': str(trcycle_ini)+'to'+str(trcycle_fin),
                   'target': target,
                   ...
                   'metrics_train': ';'.join(["roc_curve=" + str(tr_roc),
                   ...
                   'metrics_test': ';'.join(["roc_curve=" + str(tt_roc),
                                             "avg_score=" + str(tt_avgScore),
                   ...
                   'author_login': 'bgmerin1',
                   'extra_info': ';'.join(['horizon='+str(horizon),
                                           'description='+description,
                                           'processing_time='+str(elapsed_time)]),
                   'scores_extra_info_headers':';'.join(["horizon", "risk", "decile"]),
                   'year': year_partition,
    ...
    tt_predsOut = (df_scores.withColumn('model_name', lit('churn_preds_fbb'))
                   ...
                   .withColumn('executed_at', lit(executedAt_str))
                   ...
                   .withColumn('model_executed_at', lit(executedAt_str))
                   .withColumn('predict_closing_date', lit(ttcycle_ini))
                   .withColumn('year', lit(year_partition))
                   .withColumn('month', lit(month_partition))
                   .withColumn('day', lit(day_partition))
                   .withColumn('time', lit(int(str(nowMoment.hour) + str(nowMoment.minute) + str(nowMoment.second))))
                   .withColumn('prediction', col("decile").cast("string"))
