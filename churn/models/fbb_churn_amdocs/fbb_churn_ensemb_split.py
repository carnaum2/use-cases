...
if __name__ == "__main__":
    ...
    tr_ensemble = balance_df2(tr_ensemble_, 'label')
    ...
    use_var, bill_var, inter_var, spinners_var, serv_var, penal_var = getFeatGroups_fbb(spark, numerical_feats, 100, 0)
    grouped_vars = [use_var, bill_var, inter_var, spinners_var, serv_var, penal_var]
    groups = ['Use', 'Billing', 'Interactions', 'Spinners', 'Engagement', 'Penalization']
    ...
    df_aux = ttdf
    df_aux_tr = tr_ensemble
    df_aux_val = valdf

    g = 0
    for group in grouped_vars:
        
        print" Training model with {} features".format(groups[g])
        
        featCols = list(set(group) - set(noninf_feats))
        print 'Number of feats to train model {}: {}'.format(g+1, len(featCols))
        ...
        top_10 = [name[0] for name in feat_importance[:10]]
        ...
        print" Model with {} features: area under ROC(tr) = {}".format(groups[g], str(trmetrics.areaUnderROC))
        print" Model with {} features: area under ROC(tt) = {}" .format(groups[g], str(ttmetrics.areaUnderROC))
        ...
        df_aux = model.transform(df_aux.drop(col('features'))).withColumn("model_score", getScore(col("probability")).cast(DoubleType()))
        df_aux = calibmodel[0].transform(df_aux)
        df_aux = df_aux.drop(col('probability')).drop(col('prediction')).drop(col('rawPrediction')).drop(col('model_score'))\
        .withColumnRenamed('calib_model_score', 'calib_model_score_' + str(g))

        df_aux_tr = model.transform(df_aux_tr.drop(col('features'))).withColumn("model_score", getScore(col("probability")).cast(DoubleType()))
        df_aux_tr = calibmodel[0].transform(df_aux_tr)
        df_aux_tr = df_aux_tr.drop(col('probability')).drop(col('prediction')).drop(col('rawPrediction'))\
        .drop(col('model_score')).withColumnRenamed('calib_model_score', 'calib_model_score_' + str(g))
        
        df_aux_val = model.transform(df_aux_val.drop(col('features'))).withColumn("model_score", getScore(col("probability")).cast(DoubleType()))
        df_aux_val = calibmodel[0].transform(df_aux_val)
        df_aux_val = df_aux_val.drop(col('probability')).drop(col('prediction')).drop(col('rawPrediction'))\
        .drop(col('model_score')).withColumnRenamed('calib_model_score', 'calib_model_score_' + str(g))

        final_cols.append('calib_model_score_' + str(g))
        final_cols = final_cols + top_10
        g = g + 1

        
    print('Final Model Features: ')
    print(final_cols)
    assembler3 = VectorAssembler(inputCols=final_cols, outputCol="features")
    ...
    ############ FINAL MODEL TRAIN ############
    model_f = pipeline3.fit(df_aux_tr.drop(col('features')))
    calibmodel_final = get_calibration_function2(spark, model_f, df_aux_val.drop(col('features')), 'label', 10)
    
    ############ TRAIN SUBSET ############
    df_aux_tr_pred = model_f.transform(df_aux_tr.drop(col('features'))).drop(col('features'))
    df_aux_tr_pred_score = df_aux_tr_pred.withColumn("model_score", getScore(col("probability")).cast(DoubleType()))
    df_aux_tr_pred_score_calib = calibmodel_final[0].transform(df_aux_tr_pred_score).withColumnRenamed('calib_model_score', "final_score_calib")\
  .drop(col("model_score")).drop(col("probability")).drop(col("features")).drop(col('prediction')).drop(col('rawPrediction'))#\

    ############ TEST SUBSET ############
    df_aux_pred = model_f.transform(df_aux.drop(col('features'))).drop(col('features'))
    df_aux_pred_score = df_aux_pred.withColumn("model_score", getScore(col("probability")).cast(DoubleType()))
    df_aux_pred_score_calib = calibmodel_final[0].transform(df_aux_pred_score).withColumnRenamed('calib_model_score', 'final_score_calib')\
    .drop(col("model_score")).drop(col("probability")).drop(col("features")).drop(col('prediction')).drop(col('rawPrediction'))  # \

    #final_tr_df_with_calib_scores = df_aux_tr_pred_score_calib.withColumnRenamed('calib_model_score', "final_score_calib")
    tr_PredictionAndLabels = df_aux_tr_pred_score_calib.select(['final_score_calib', 'label']).rdd.map(lambda r: (r['final_score_calib'], r['label']))
    
    add_cols = df_aux_tr_pred_score_calib.withColumn('maximo', greatest(*final_cols)).withColumn('minimo', least(*final_cols))
    
    for name in ['maximo', 'minimo']:
        tt_ensemble = add_cols.select([name, 'label']).rdd.map(lambda r: (r[name], r['label']))
        ttmetrics = BinaryClassificationMetrics(tt_ensemble)
        print "Column {}: Area under ROC(tt) = {}".format(name, str(ttmetrics.areaUnderROC))
    ...
    tt_PredictionAndLabels = df_aux_pred_score_calib.select(['final_score_calib', 'label']).rdd.map(
    ...
    print '############## Final ensemble AUC (tr): {} ##############'.format(tr_metrics_final.areaUnderROC)
    print '############## Final ensemble AUC (tt): {} ##############'.format(tt_metrics_final.areaUnderROC)
    print(" ")
