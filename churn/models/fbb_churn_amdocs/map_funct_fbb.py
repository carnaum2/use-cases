def getFeatGroups_fbb(spark, numerical_feats, num_feats = 20, mode = 0):
    ...
    penalization = tgs + incremental_penal
    interactions = order + campaigns + ccc + pbms
    use = apps + incremental_data
    billing = incremental_bill + additional_bill
    engagement = incremental_services + additional_services + additional_tariff + device
    spinners = navigation + additional_days_since
