...
if __name__ == "__main__":
    ...
    vec_selected_1s = tr_1s.randomSplit(arr)
    vec_selected_0s = tr_0s.randomSplit(arr)
    arr = np.ones(num_tr_dfs)

    vector_train = []
    for i in range(0, num_tr_dfs):
        value = vec_selected_1s[i].count()
        tr = vec_selected_1s[i].union(vec_selected_0s[i].limit(value))
        print 'Size of training df number {}: {}'.format(i+1, value*2)
    ...
    for i in range(0, num_tr_dfs):
        ...
        print'Predictions and calibration for training subset of model {}'.format(i + 1)
    ...
    for i in range(0, num_tr_dfs): 
        ...
        print'Binary metrics for training subset of model {}'.format(i + 1)
    ...
    for i in range(0, num_tr_dfs):
        ...
        print'Test predictions for model {}'.format(i + 1)
    ...
    for i in range(0, num_tr_dfs):
        ...
        print(i)
        auc_tr = tr_metrics_vec[i].areaUnderROC
        auc_tt = ttmetrics.areaUnderROC 
        if auc_tt > max_val:
            max_idx = i
            max_val = auc_tt
            
        print "Model {}: Area under ROC(tr) = {}".format(i, auc_tr)
        print "Model {}: Area under ROC(tt) = {}".format(i, auc_tt)
        
    print "Max AUC for Test Subset = {}".format(i, max_val)
