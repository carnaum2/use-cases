...
from pyspark.sql.functions import udf, concat, size, col, lpad, struct, count as sql_count, lit, min as sql_min, max as sql_max, collect_list, when, desc, asc, create_map, sum as sql_sum
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', '--trigger', metavar='trigger_name:orders, billing>', type=str, required=True,
                        help='orders,billing')
    ...
    from churn.analysis.triggers.orders.get_ids_for_analysis import get_sel_cols
    ...
    elif trigger == "billing":
        from churn.analysis.triggers.billing.run_segment_billing import get_car
        df_tar, refprevalence, refprevalence_all = get_car(spark, closing_day, labeled_mini_ids=True, rule_nb=None, force_gen=False)

    else:
        import sys
        print("Unknown trigger {}".format(trigger))
        sys.exit(1)
