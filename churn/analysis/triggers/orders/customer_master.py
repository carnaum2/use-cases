...
PATH_LABELED_GAP = "/data/attributes/vf_es/trigger_analysis/customer_master_gap/"
PATH_UNLABELED_GAP = "/data/attributes/vf_es/trigger_analysis/customer_master_unlabeled_gap/"
...
def get_customer_saving_path_gap(unlabeled):
    path_to_save = PATH_LABELED_GAP if not unlabeled else PATH_UNLABELED_GAP
    return path_to_save
...
def getFbbDxsForCycleList_anyday(spark, closing_day, closing_day_target):
    ...
    target_base = (get_segment_msisdn_anyday(spark, closing_day_target, cod = "03")
                 #   .filter((col("clase_cli_cod_clase_cliente") == "RS") & (col("cod_estado_general").isin("01", "09")) & (col("rgu").isNotNull()))
                    .filter(col('rgu')=='fbb')
                    .select("msisdn")
                    .distinct())
...
def get_target(spark, closing_day):
    ...
    if move_date_n_days(closing_day, n=(28 + 3)) > today_str:
        print("ERROR: closing_day [{}] + churn_window [{}] must be lower than today [{}] - 3 days".format(closing_day, 28, today_str))
    ...
    df_target_nifs = (df_services.join(df_sopo_fix, ['msisdn'], "left").na.fill({'label_srv': 0.0})
                      ...
                      .withColumn('reason', get_churn_reason_udf(col('dates_array'))).drop("tmp", "tmp_date0"))
    ...
    df_target_nifs = df_target_nifs.cache()
    print(df_target_nifs.count(),
          df_target_nifs.select("nif_cliente").distinct().count(),
          df_target_nifs.select("nif_cliente", "label", "churn_date", "days_until_churn", "reason").drop_duplicates().count())
...
def get_segment_msisdn_anyday(spark, anyday, cod = "99"):

    from churn.datapreparation.general.customer_base_utils import  get_customer_base_segment

    df_base = get_customer_base_segment(spark, date_=anyday, cod = cod)
...
def get_segment_nif_anyday(spark, anyday):
    ...
    df_base = df_base.select("nif_cliente", "segment_nif").drop_duplicates(["nif_cliente"])
...
def get_customer_master(spark, closing_day, unlabeled):
    '''
    The difference with respect to get_customer_master_module is that this function return the saved module or create a new one
    :param spark:
    :param closing_day:
    :param unlabeled:
    :return:
    '''
    saving_path = get_customer_saving_path(unlabeled) + "year={}/month={}/day={}".format(int(closing_day[:4]), int(closing_day[4:6]), int(closing_day[6:]))
    ...
    if check_hdfs_exists(saving_path):
        ...
        df_base = get_customer_master_module(spark, closing_day, unlabeled=unlabeled, save=True)
        print("Generated {} for closing_day {}".format(saving_path, closing_day))

    return df_base

def get_customer_master_gap(spark, closing_day, unlabeled):
    '''
    The difference with respect to get_customer_master_module is that this function return the saved module or create a new one
    :param spark:
    :param closing_day:
    ...
    :return:
    '''
    saving_path = get_customer_saving_path_gap(unlabeled) + "year={}/month={}/day={}".format(int(closing_day[:4]), int(closing_day[4:6]), int(closing_day[6:]))
    print("Looking customer master in {}".format(saving_path))
    from pykhaos.utils.hdfs_functions import check_hdfs_exists
    if check_hdfs_exists(saving_path):
        print("Found already a customer saving path - '{}'".format(saving_path))
        ...
        print("Not found {}. Generating...".format(saving_path))
        df_base = get_customer_master_module_gap(spark, closing_day, unlabeled=unlabeled, save=True)
        print("Generated {} for closing_day {}".format(saving_path, closing_day))
...
def get_customer_master_module(spark, closing_day, unlabeled=False, save=True):
    ...
    print(" - - - - - - - - CUSTOMER MASTER __ LABEL - - - - - - - - ")
    ...
    if unlabeled:
        ...
        print("Getting labeled car")
        from churn.analysis.triggers.base_utils.base_utils import get_churn_target_nif
        df_target_nifs = get_churn_target_nif(spark, closing_day)
        #df_target_nifs = get_target(spark, closing_day)  # id=nif_cliente,num_cliente "label" (aggregada por nif)

    print(" - - - - - - - - CUSTOMER MASTER __ BASE (cycles - 2) - - - - - - - - ")
    from pykhaos.utils.date_functions import move_date_n_cycles, move_date_n_days, get_previous_cycle
    closing_day_n2 = move_date_n_days(closing_day, n=-14)
    print("Using {} for comparing rgus".format(closing_day_n2))
    ...
    for ii in range(0,3):
        try:
            df_tgs = get_tgs(spark, closing_day_tgs).select("nif_cliente", 'tgs_days_until_fecha_fin_dto','tgs_days_until_f_fin_bi', 'tgs_has_discount',
    ...
    df_join2 = df_join2.fillna(-1, subset=["tgs_days_until_f_fin_bi", "tgs_has_discount"])
...
def get_customer_master_module_gap(spark, closing_day, unlabeled=False, save=True):
    '''
    ...
    if unlabeled:
        ...
        from churn.analysis.triggers.base_utils.base_utils import get_churn_target_nif_gap
        print("Getting labeled car")
        df_target_nifs = get_churn_target_nif_gap(spark, closing_day)  # id=nif_cliente,num_cliente "label" (aggregada por nif)
    ...
    for ii in range(0,3):
        try:
            df_tgs = get_tgs(spark, closing_day_tgs).select("nif_cliente", 'tgs_days_until_f_fin_bi', 'tgs_has_discount',
                                                    'tgs_target_accionamiento').drop_duplicates()
            print("Found extra feats for tgs - {}".format(closing_day_tgs))
...
def save_customer_module(df_agg, closing_day, path_to_save):
    ...
    print("Started saving customer module for closing_day={} path={}".format(closing_day, path_to_save))
    (df_agg.write.partitionBy('year', 'month', 'day').mode("append").format("parquet").save(path_to_save))

    print("Saved {} for closing_day {} - {}".format(path_to_save, closing_day, path_to_save))
...
if __name__ == "__main__":
    ...
    for closing_day in closing_day_list:
        ...
        df_agg = get_customer_master(spark, closing_day, unlabeled)
