...
def create_model_output_dataframes(spark, predict_closing_day, df_model_scores, model_params_dict, model_name, extra_info_cols=None):
    ...
    df_model_scores = df_model_scores.withColumn('msisdn', lit("")).withColumn('client_id', lit("")).withColumnRenamed('NIF_CLIENTE', 'nif')
    ...
    df_model_scores = (df_model_scores
                       .select(*(
            ["nif", "msisdn", "client_id", "executed_at", "model_executed_at", "predict_closing_date", "model_output", "scoring", "prediction", "extra_info", "time", "model_name", "year", "month", "day"])
                               )
                       )
...
if __name__ == "__main__":
    ...
    starting_day = "20191010"
    closing_day = "20191010"
    ...
    print '[Info] Input dates (string): year = ' + closing_day[0:4] + ' - month = ' + closing_day[4:6] + ' - day = ' + closing_day[6:8]
    print '[Info] Input dates (int): year = ' + str(int(closing_day[0:4])) + ' - month = ' + str(int(closing_day[4:6])) + ' - day = ' + str(int(closing_day[6:8]))

    partition_date = "year={}/month={}/day={}".format(str(int(closing_day[0:4])), str(int(closing_day[4:6])), str(int(closing_day[6:8])))
    ...
    load_reasons = '/data/udf/vf_es/churn/churn_reasons/trigger_reasons_results/year={}/month={}/day={}'.format(int(closing_day[:4]), int(closing_day[4:6]), int(closing_day[6:8]))
    ...
    df_alv_reasons = df_reasons.where(col('model') == 'asaezco')
    df_cris_reasons = df_reasons.where(col('model') == 'csanc109')
    print'Loaded Reasons Dataframes'

    print '[Info triggers_join] df_alv_reasons - num rows: ' + str(df_alv_reasons.count())
    ...
    print '[Info triggers_join] df_full_alv - num rows: ' + str(df_full_alv.count())
    print '[Info triggers_join] df_full_cris - num rows: ' + str(df_full_cris.count())
    ...
    save_dir = '/data/udf/vf_es/churn/triggers/scores_and_reasons/year={}/month={}/day={}'.format(int(closing_day[0:4]), int(closing_day[4:6]), int(closing_day[6:8]))
    ...
    model_params_dict = {"model_level": ["nif"],
                         ...
                         "target": ['full car'],
                         "model_path": [" /data/udf/vf_es/churn/triggers/car_tr && /data/udf/vf_es/churn/triggers/car_exploration_model1"],
                         ...
                         "extra_info": ["top0_reason | top1_reason | top2_reason | top3_reason | top4_reason | Incertidumbre"],
                         }

    model_name = 'triggers_ml'

    df_model_scores = df_final.withColumnRenamed('calib_model_score', 'scoring').drop('executed_at','model','closing_day')

    df_model_scores.printSchema()

    df_parameters, df_model_scores = create_model_output_dataframes(spark, closing_day, df_model_scores, model_params_dict, model_name, extra_info_cols=EXTRA_INFO_COLS)
