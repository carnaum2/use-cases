...
def getTopNFeatsK2(spark, df, n_feats, filtered_cols):
    ...
    for i in range(0, n_feats):
        ...
        name_val = 'top{}_val'.format(i)
        df = df \
            .withColumn(name_col, when(df[name_val] > 0.1, df[name_feat]).otherwise('-'))
    return (df)
...
if __name__ == "__main__":
    ...
    # Save directory
    save_dir = '/data/udf/vf_es/churn/churn_reasons/trigger_reasons_results/year={}/month={}/day={}'.format(int(closing_day[:4]),int(closing_day[4:6]),int(closing_day[6:8]))
    save_dir_full = '/data/udf/vf_es/churn/churn_reasons/trigger_reasons_results_full/' + args.model + '/year={}/month={}/day={}'.format(int(closing_day[:4]),int(closing_day[4:6]),int(closing_day[6:8]))
    # Train and Test Dataframes
    print'############ Loading Train dataframe for training day {} and test for closing day ############'.format(
        starting_day, closing_day)
    if args.model == 'asaezco':
        path_train = '/data/udf/vf_es/churn/triggers/car_tr_filtered/year={}/month={}/day={}'.format(int(starting_day[:4]),
        ...
        df_car = get_trigger_minicar3(spark, closing_day)
    else:
        ...
        from churn.analysis.triggers.trigger_billing_ccc.trigger_smart_exploration_production import get_exploration_car_unlabeled
        ...
        df_car = get_exploration_car_unlabeled(spark, closing_day, None)
    ...
    if args.model == 'asaezco':
        ...
        path_feats = '/data/udf/vf_es/churn/triggers/feat_imp_filtered/year={}/month={}/day={}'.format(int(starting_day[:4]), int(starting_day[4:6]), int(starting_day[6:8]))
    ...
    table_meta_name = '/data/udf/vf_es/churn/metadata/metadata_v1.1'
    table_meta = spark.read.option("delimiter", "|").option("header", True).csv(table_meta_name)
    print("############# Finished Loading Metadata Table ############")
    '''

    orders_path = '/data/attributes/vf_es/trigger_analysis/orders_sla_csanc109/year=' + str(starting_day[0:4]) + '/month=' + str(int(starting_day[4:6])) + '/day=' + str(int(starting_day[6:8]))
    sample_orders = spark.read.load(orders_path)

    from churn.analysis.triggers.ml_triggers.metadata_triggers import get_billing_metadata, get_tickets_metadata, get_reimbursements_metadata, get_ccc_metadata, get_customer_metadata, get_orders_metadata, get_full_billing_metadata
    if args.model == 'asaezco':
    ...
    table_meta = metadata_billing.union(metadata_tickets).union(metadata_reimb).union(metadata_ccc).union(metadata_cust).union(metadata_orders)
    ...
    score_orders = [x + '_score' for x in orders_var if x in relevant_var]
    score_reimb = [x + '_score' for x in reimb_var if x in relevant_var]
    score_tickets = [x + '_score' for x in tickets_var if x in relevant_var]
    score_serv = [x + '_score' for x in ccc_var if x in relevant_var]
    ...
    # Df to save results
    #relevant_var = list(set(relevant_var) - set(['NUM_TICKETS_TIPO_INF', 'NUM_TICKETS_TIPO_REC']))
    ...
    for name_ in relevant_var:
        ...
        df_aux = df_aux.withColumn(name_ + '_score', when(df_aux[name_] != null_vals[j], 0.00000000001 * randn(seed) + (p_1s * df_aux[name_ + '_p1']) / (p_1s * df_aux[name_ + '_p1'] + p_0s * df_aux[name_ + '_p0'])).otherwise(0.0))
        df_aux = df_aux.withColumn(name_ + '_score', col(name_ + '_score').cast('double'))
        df_aux = df_aux.withColumn(name_ + '_score', 0.00000000001 * randn(seed) + (p_1s * df_aux[name_ + '_p1']) / (p_1s * df_aux[name_ + '_p1'] + p_0s * df_aux[name_ + '_p0']))
        j = j + 1
    ...
    for name_ in score_cols_2:
        seed = np.random.randint(0, 500000)
        df_aux = df_aux.withColumn(name_, 0.000000001 * randn(seed) + col(name_))

    save_df = df_aux

    #save_df.withColumn('model',lit(args.model)).repartition(300).write.save(save_dir_full, format='parquet', mode='append')
    ...
    df_final = df_final.withColumn('Incertidumbre', when(df_final['top0_val'] < 1,
                                                         -df_final['top0_val'] * log2(df_final['top0_val']) - (
                                                                     1 - df_final['top0_val']) * log2(1 - df_final['top0_val'])).otherwise(0))
    ...
    FUNCTION TO AVOID REPEATED REASONS SEPARATED BY COMMAS
    
    def process_array3(a):
        a_list = a.split(";")
        r = []
        cont = 0
        for e in a_list:
            if(',' in e):
                r.append(e.split(',')[cont])
                cont = 1
            else:
                r.append(e)
        return ";".join(r)
        
    process_array3_udf = udf(lambda v: process_array3(v), StringType())
