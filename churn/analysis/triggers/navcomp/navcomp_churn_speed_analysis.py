...
def lost_customer_analysis(spark, deliv_date_):
    ...
    deliv_all_df = spark \
        .read \
        .parquet(
        "/data/attributes/vf_es/model_outputs/model_scores/model_name=triggers_navcomp/year=" + deliv_year_ + "/month=" + deliv_month_ + "/day=" + deliv_day_) \
        .withColumn('scoring', col('scoring') + lit(0.00001) * randn())

    selected = deliv_all_df.orderBy(desc('scoring')).head(50000)
    schema = deliv_all_df.schema
    deliv_df = spark.createDataFrame(selected, schema=schema)
...
def days_to_churn_analysis(spark, deliv_date_):
    ...
    lab_deliv_df = deliv_df.withColumnRenamed('nif', 'nif_cliente').join(target_df, ['nif_cliente'], 'inner') \
        .withColumn('days_to_port', datediff(col('portout_date'), from_unixtime(unix_timestamp(lit(picture_date_), 'yyyyMMdd')))) \
