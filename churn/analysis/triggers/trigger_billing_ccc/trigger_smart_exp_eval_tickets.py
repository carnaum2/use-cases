...
CAR_PATH = "/data/udf/vf_es/churn/triggers/car_exploration_model1_tickets/"
CAR_PATH_UNLABELED = "/data/udf/vf_es/churn/triggers/car_exploration_unlabeled_model1_tickets/"
...
if __name__ == "__main__":
    ...
    numeric_columns = list(set(EXPLORATION_COLS) - {"blindaje", "nif_cliente", "segment_nif"}) +  [name_ for name_ in df_tr_all.columns if 'NUM_TICKETS' in name_]
    get_feat_auc(spark, tr_input_df, numeric_columns, 'label')
    ...
    get_metrics(df_tt_preds, title="TEST", nb_deciles=None, score_col="model_score", label_col="label", refprevalence=tr_churn_ref)
    ...
    path_to_save = "/data/udf/vf_es/churn/triggers/model1_50k_filtered/"
    start_time = time.time()
