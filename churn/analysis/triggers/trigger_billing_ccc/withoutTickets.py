...
def build_exploration_car(spark, closing_day, select_cols, labeled=True):
    ...
    print('[Info build_exploration_car] Number of discarded customers because of rgu decrements: ') + str(
        base_join - base_rgu)

    metadata_ccc = get_ccc_metadata(spark)
    ccc_map_tmp = metadata_ccc.select("feature", "imp_value", "type").rdd.map(
        lambda x: (x["feature"], x["imp_value"], x["type"])).collect()
    ccc_map = dict([(x[0], str(x[1])) if x[2] == "categorical" else (x[0], float(x[1])) for x in ccc_map_tmp if
                    x[0] in tr_ccc_all.columns])

    tr_ccc_base = tr_base.join(tr_ccc_all, ['nif_cliente'], 'left').na.fill(ccc_map)

    tr_input_df = tr_ccc_base.filter((col('CHURN_CANCELLATIONS_w8') == 0)).select(
        bill_cols + ['nif_cliente', 'blindaje', 'nb_rgus', 'nb_tv_services_nif', 'label', 'segment_nif'])
    ...
    orders_map_tmp = orders_meta.select("feature", "imp_value", "type").rdd.map(
        lambda x: (x["feature"], x["imp_value"], x["type"])).collect()
    orders_map = dict([(x[0], str(x[1])) if x[2] == "categorical" else (x[0], float(x[1])) for x in orders_map_tmp if
                       x[0] in tr_orders.columns])
    ...
    print("Filtered NIFs with forbidden orders before={} after={} eliminados={}".format(before_cust, after_cust,
                                                                                        before_cust - after_cust))
    ...
    tr_reimb_df = spark.read.parquet(
        '/data/attributes/vf_es/trigger_analysis/reimbursements/' + partition_date).withColumnRenamed("NIF_CLIENTE",
                                                                                                      "nif_cliente")
    ...
    billing_map_tmp = metadata_billing.select("feature", "imp_value", "type").rdd.map(
        lambda x: (x["feature"], x["imp_value"], x["type"])).collect()

    billing_map = dict([(x[0], str(x[1])) if x[2] == "categorical" else (x[0], float(x[1])) for x in billing_map_tmp if
                        x[0] in tr_billing_df.columns])
...
if __name__ == "__main__":
    ...
    df = df.withColumn('flag_billshock', when(col('greatest_diff_bw_bills') > 30, 1).otherwise(0))
    ...
    print'Size of the tt car (only with problems): ' + str(df.count())
    df_tr_all = df.where((col('flag_reimb')> 0) |(col('flag_billshock')> 0)|(col('flag_fact')> 0)|(col('flag_tickets')> 0)|(col('flag_orders')> 0))
    print'Size of the tt car (only with problems): ' + str(df_tr_all.count())

    print'Churn rate for customers with reimbursements: ' + str(df.where(col('flag_reimb') > 0).select(sql_avg('label').alias('churn_rate')).rdd.first()['churn_rate'])
    print'Churn rate for customers with billshock: ' + str(df.where(col('flag_billshock') > 0).select(sql_avg('label').alias('churn_rate')).rdd.first()['churn_rate'])
    print'Churn rate for customers with orders: ' + str(df.where(col('flag_orders') > 0).select(sql_avg('label').alias('churn_rate')).rdd.first()['churn_rate'])
    print'Churn rate for customers with tickets: ' + str(df.where(col('flag_tickets') > 0).select(sql_avg('label').alias('churn_rate')).rdd.first()['churn_rate'])
    print'Churn rate for customers with wrong fact: ' + str(df.where(col('flag_fact') > 0).select(sql_avg('label').alias('churn_rate')).rdd.first()['churn_rate'])
    ...
    unlab_tt_input_df = unlab_tt_input_df.withColumn('NUM_TICKETS',col('NUM_TICKETS_TIPO_INC_w8') + col('NUM_TICKETS_TIPO_REC_w8') + col('NUM_TICKETS_TIPO_AV_w8'))

    df = unlab_tt_input_df
    df = df.withColumn('flag_reimb', when(col('Reimbursement_adjustment_net') != 0, 1).otherwise(0))
    ...
    rem_feats = get_feat_auc(spark, tr_input_df, [name_ for name_ in df_tr_all.columns if 'NUM_TICKETS' in name_],
                             'label')

    numeric_columns = list(set(EXPLORATION_COLS) - {"blindaje", "nif_cliente", "segment_nif"}) + ['NUM_TICKETS', 'NUM_TICKETS_TIPO_FACT_w8']
    ...
    path_to_save = "/data/udf/vf_es/churn/triggers/model1_50k_withoutTickets/"
    start_time = time.time()
