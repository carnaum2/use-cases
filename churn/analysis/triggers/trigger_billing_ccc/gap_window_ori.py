...
if __name__ == "__main__":
    ...
    '''
    df_tr_all = get_exploration_car_unlabeled(spark, tr_date_, EXPLORATION_COLS)
    df_tr_all = df_tr_all.drop('label')
    ##########################################################################################
    # 2.1 Labeling with gap window
    ##########################################################################################

    print("Getting labeled car")
    df_target_nifs_tr = get_churn_target_nif_gap(spark, tr_date_, 30, 5)

    df_tr_all = df_tr_all.join(df_target_nifs_tr.select('NIF_CLIENTE','label','portout_date'),['NIF_CLIENTE'], 'inner')

    from pykhaos.utils.date_functions import move_date_n_days

    closing_day_5 = move_date_n_days(tr_date_, n=5)
    from datetime import datetime
    import datetime
    closing_day_date_5m = datetime.date(int(closing_day_5[:4]), int(closing_day_5[4:6]), int(closing_day_5[6:8]))
    df_tr_all = df_tr_all.where((col('portout_date') > closing_day_date_5m) | col('portout_date').isNull())
    tr_churn_ref = 100.0 * df_tr_all.select(sql_avg('label').alias('churn_rate')).rdd.first()['churn_rate']
    '''
    path_train = CAR_PATH + PARTITION_DATE.format(int(tr_date_[:4]), int(tr_date_[4:6]), int(tr_date_[6:]))
    print('################## Loading tr df ##################')


    df_tr_all = spark.read.load(path_train)
    tr_churn_ref = 100.0 * df_tr_all.select(sql_avg('label').alias('churn_rate')).rdd.first()['churn_rate']
    ...
    df_tr_all = df.where((col('flag_reimb')> 0) |(col('flag_fact')> 0)|(col('flag_tickets')> 0)|(col('flag_orders')> 0)|(col('flag_billshock')> 0))
    ...
    df_tt_all = get_exploration_car_unlabeled(spark, tt_date_, EXPLORATION_COLS)
    df_tt_all = df_tt_all.drop('label')

    df_target_nifs_tt = get_churn_target_nif_gap(spark, tt_date_, 30, 5)

    df_tt_all = df_tt_all.join(df_target_nifs_tt.select('NIF_CLIENTE', 'label', 'portout_date'), ['NIF_CLIENTE'], 'inner')

    from pykhaos.utils.date_functions import move_date_n_days

    closing_day_5 = move_date_n_days(tt_date_, n=5)
    from datetime import datetime
    import datetime
    df_tt_all_ori = df_tt_all

    closing_day_date_5m = datetime.date(int(closing_day_5[:4]), int(closing_day_5[4:6]), int(closing_day_5[6:8]))
    df_tt_all = df_tt_all#.where((col('portout_date') > closing_day_date_5m) | col('portout_date').isNull())

    tt_churn_ref = 100.0 * df_tt_all.select(sql_avg('label').alias('churn_rate')).rdd.first()['churn_rate']
    ...
    df_tt_all = df.where((col('flag_reimb')> 0) |(col('flag_fact')> 0)|(col('flag_tickets')> 0)|(col('flag_orders')> 0)|(col('flag_billshock')> 0))
    #(col('flag_fact') > 0) | (col('flag_orders') > 0) | (col('flag_tickets') > 0)|
    #|(col('flag_billshock')> 0)
    df_tt_all = df_tt_all.cache()
    # (col('flag_billshock') > 0) |
    ...
    tt_churn_ref = tt_churn_ref
    for i in top_cust:
        print(i)
        selected = df_top.head(i)
        selected_df = spark.createDataFrame(selected, schema=schema)
        total_churners_trigger = selected_df.where(col('label') > 0).where((col('portout_date') > closing_day_date_5m)).count()
