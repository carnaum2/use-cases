...
log_file=PRC_EXP_PRODUCTOS_CUENTA
...
if [ $# -lt $num_arguments ]; then
	...
	echo '1. Pair YearMonth without spaces'
        ...
        echo ">>>>>>>ERROR<<<<<<< Example: ./PRC_EXP_PRODUCTOS_CUENTA.sh 201610"
        echo -e "\n"
        exit -1
...
# start=$SECONDS
# hive -v -hiveconf MONTH0=$1 -i $ruta_ql/PRC_EXP_PRODUCTOS_CUENTA.hql
# duration=$(($SECONDS))
...
hive -hiveconf hive.log4j.file=$HOME/hive-log4j_${log_file}.properties -hiveconf MONTH0=$1 -i $ruta_ql/PRC_EXP_PRODUCTOS_CUENTA.hql #>> $ruta_log/PRC_EXP_PRODUCTOS_CUENTA.log
