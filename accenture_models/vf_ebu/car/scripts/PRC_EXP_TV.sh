...
if [ $# -lt $num_arguments ]; then
	...
	echo '1. Pair YearMonth without spaces'
        ...
        echo ">>>>>>>ERROR<<<<<<< Example: ./PRC_EXP_TV.sh 201610"
        echo -e "\n"
        exit -1
...
if [ ${montht1} -lt 10 ]; then
					MT1=${yeart1}0${montht1}
else
					MT1=${yeart1}${montht1}
fi
...
hive -hiveconf hive.log4j.file=$HOME/hive-log4j_${log_file}.properties -hiveconf MONTH0=$M0 -hiveconf MONTH1=$M1 -hiveconf MONTH2=$M2 -hiveconf MONTH3=$M3 -hiveconf MONTHT1=$MT1 -i $ruta_ql/PRC_EXP_TV.hql #>> $ruta_log/PRC_EXP_TV.log
