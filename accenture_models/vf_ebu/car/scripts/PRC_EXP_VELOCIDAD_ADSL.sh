...
log_file=PRC_EXP_VELOCIDAD_ADSL 
...
if [ $# -lt $num_arguments ]; then
	...
	echo '1. Pair YearMonth without spaces'
        ...
        echo ">>>>>>>ERROR<<<<<<< Example: ./PRC_EXP_VELOCIDAD_ADSL.sh 201610"
        echo -e "\n"
        exit -1
...
hive -hiveconf hive.log4j.file=$HOME/hive-log4j_${log_file}.properties -hiveconf MONTH0=$M0 -hiveconf MONTH1=$M1 -hiveconf MONTH2=$M2 -i $ruta_ql/PRC_EXP_VELOCIDAD_ADSL.hql #>> $ruta_log/PRC_EXP_VELOCIDAD_ADSL.log

duration=$(($SECONDS))

echo 'DuraciÃ³n: ' $(($duration/60)) 'min.'
