...
FROM INPUT.VF_POS_AC_FINAL WHERE PARTITIONED_MONTH =  '${hiveconf:MONTH0}') AC_FIN
...
INNER JOIN 
(SELECT DISTINCT X_ID_RED AS MSISDN
FROM INPUT.VF_POS_AC_FINAL WHERE PARTITIONED_MONTH =  '${hiveconf:MONTH1}') AC_FIN
...
set mapred.job.name = WORK_LLAMADAS_USAC;
DROP TABLE WORK.WORK_LLAMADAS_USAC;
CREATE TABLE WORK.WORK_LLAMADAS_USAC AS 
SELECT 
GRP4.MSISDN, 
MAX(GRP4.MES) AS MES,
...
set mapred.job.name = PAR_EXP_USAC_LIN;
DROP TABLE VF_POSTPAID.PAR_EXP_USAC_LIN;
CREATE TABLE VF_POSTPAID.PAR_EXP_USAC_LIN AS
SELECT
B.*,
A.NIF,
...
(SELECT 
X_ID_RED AS MSISDN, 
MAX(X_NUM_IDENT) AS NIF 
FROM INPUT.VF_POS_AC_FINAL WHERE PARTITIONED_MONTH='${hiveconf:MONTH0}'
GROUP BY X_ID_RED) A
INNER JOIN
WORK.WORK_USAC_LINEAS B
ON A.MSISDN = B.MSISDN
LEFT JOIN WORK.WORK_LLAMADAS_USAC C 
ON A.MSISDN=C.MSISDN;


set mapred.job.name = PAR_EXP_USAC_CLI;
DROP TABLE VF_POSTPAID.PAR_EXP_USAC_CLI;
CREATE TABLE VF_POSTPAID.PAR_EXP_USAC_CLI AS
SELECT
NIF,
...
DROP TABLE WORK.WORK_USAC_TT3;
DROP TABLE WORK.WORK_USAC_LINEAS;
DROP TABLE WORK.WORK_LLAMADAS_USAC_${hiveconf:MONTH0};
