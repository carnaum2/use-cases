...
if [ $# -lt $num_arguments ]; then
	...
	echo '1. Pair YearMonth without spaces'
        ...
        echo ">>>>>>>ERROR<<<<<<< Example: ./PRC_EXP_TACFAC.sh 201610"
        echo -e "\n"
        exit -1
...
start=$SECONDS
hive -hiveconf hive.log4j.file=$HOME/hive-log4j_${log_file}.properties -hiveconf MONTH0=$M0 -hiveconf MONTH1=$M1 -hiveconf MONTH2=$M2 -hiveconf MONTH3=$M3 -i $ruta_ql/PRC_EXP_TACFAC.hql #>> $ruta_log/PRC_EXP_TACFAC.log
duration=$(($SECONDS))
