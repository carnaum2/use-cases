...
main <- function(car.month, rem.month, sol.month, sol.month.2) {
  ...
  cat("Number of port-outs: ", nrow(dt.train[CLASS == 1]), " from ", nrow(dt.solicitudes), "\n")
  
  ifile <- paste0(cvm.data.folder, sol.month.2, "/dt.solicitudes.RData")
  cat("[LOAD]", ifile, "\n")
  load(ifile)
  ...
  ofile <- paste0(churn.datasets.folder, "/dt.train.mo.", car.month, ".", sol.month,".RData")
  cat("[SAVE] ", ofile, "\n")
