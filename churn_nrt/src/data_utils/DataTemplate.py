...
class DataTemplate:
    ...
    def __init__(self, spark, module_name, verbose=True):
        self.SPARK = spark
        self.MODULE_NAME = module_name
        self.PATH = os.path.join(HDFS_CHURN_NRT, self.MODULE_NAME)
    ...
    def save(self, df, closing_day, overwrite=True):
        '''
        ...
        print("[DataTemplate] save | Started saving at '{}' partitioned by {} (mode={})".format(path_to_save,
                                                                                                PARTITION_DATE.format(int(closing_day[:4]),
                                                                                                    int(closing_day[4:6]),
                                                                                                    int(closing_day[6:])),
                                                                                                    mode))
        ...
        print("[DataTemplate] save | Saved {} [Elapsed time = {} minutes]".format(os.path.join(path_to_save, PARTITION_DATE.format(int(closing_day[:4]),
                                                                                 int(closing_day[4:6]),
                                                                                 int(closing_day[6:]))),
    ...
    def get_module(self, closing_day, save=True, save_others=True, force_gen=False, *args, **kwargs):
        ...
        # is_default --> to avoid returning an stored module for this closing_day when *args/**kwargs are specified
        is_default = self.is_default_module(*args, **kwargs)
        module_exists = check_hdfs_exists(module_path)
        if is_default and module_exists and not force_gen:
    ...
    def get_saving_path(self, closing_day=None, add_partition=True):
        ...
        if add_partition:
            path_to_save = os.path.join(path_to_save,  PARTITION_DATE.format(int(closing_day[:4]),
                                                                             int(closing_day[4:6]),
                                                                             int(closing_day[6:])))
