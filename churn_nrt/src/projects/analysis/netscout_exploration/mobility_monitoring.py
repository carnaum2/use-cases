...
def get_hourly_mob_df(spark, date_, area_ = 'municipality', city_ = 'Madrid', age_analysis = False):
    """
    ...
    from churn_nrt.src.utils.mobility_utils import get_cells_location2, get_age_data

    cell_df = get_cells_location2(spark, date_)

    age_df = get_age_data(spark, date_)
    ...
    data_netscout_ori = (spark
                         ...
                         .filter(lower(col(area_)).contains(city_)))

    data_netscout_ori = data_netscout_ori.join(age_df, ['msisdn'], 'left').na.fill({'age_group': 'unknown_age'}) if(age_analysis) else data_netscout_ori.withColumn('age_group', lit('none'))

    data_netscout_ori = data_netscout_ori\
        .groupBy('msisdn', 'cal_hour', 'age_group')\
        .agg(countDistinct('cell_mcc', 'cell_mnc', 'cell_area', 'cell_id').alias('num_cells'))\
        .groupBy('cal_hour', 'age_group')\
        .agg(countDistinct('msisdn').alias('volume_msisdn'), sql_avg('num_cells').alias('avg_num_cells'), sql_std('num_cells').alias('std_num_cells'), sql_max('num_cells').alias('max_num_cells'))\
    ...
    print "[Info] Hourly mobility in " + city_ + " for " + date_ + " showed above"

    result_dir = '/var/SP/data/home/jmarcoso/' + city_

    if(os.path.isdir(result_dir)==False):
        os.system('mkdir ' + result_dir)

    pd_hourly_mob.to_csv(result_dir + '/tmp_mobility_' + city_ + '_' + date_ + '.txt', sep='|', header=True, index=False)
...
if __name__ == "__main__":
    ...
    sc, spark, sql_context = get_spark_session("mobility_analysis")

    area_ = sys.argv[1]

    city_ = sys.argv[2]

    age_ = True if(sys.argv[3].lower()=='t') else False
    ...
    num_days = get_diff_days(date_ini, date_end)

    #date_ = "20200321"
    dates_ = [move_date_n_days(date_ini, d) for d in list(range(0, (num_days + 1)))]

    #dates_ = ["20200305", "20200319"]

    mobility_curve_list = [get_hourly_mob_df(spark, d_, area_, city_, age_) for d_ in dates_]

    result_df = pd.concat(mobility_curve_list, ignore_index=True)

    result_dir = '/var/SP/data/home/jmarcoso/' + city_

    result_df.to_csv(result_dir + '/mobility_evolution_' + area_ + '_' + city_ + '_' + dates_[0] + '_' + dates_[-1] + '.txt',
