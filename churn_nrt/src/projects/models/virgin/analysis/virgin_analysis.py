...
#vgddp387hr:csanc109:/var/SP/data/home/csanc109/src/devel/use-cases/churn_nrt/src/projects/models/virgin/analysis $ spark2-submit --master yarn --deploy-mode client --queue root.BDPtenants.es.medium --conf spark.dynamicAllocation.enabled=true --conf spark.dynamicAllocation.initialExecutors=4 --conf spark.dynamicAllocation.minExecutors=4 --conf spark.dynamicAllocation.maxExecutors=20 --executor-cores 4 --executor-memory 16G --driver-memory 10G --conf spark.driver.maxResultSize=32G --conf spark.yarn.am.waitTime=800000s --conf spark.network.timeout=10000001 --conf spark.executor.heartbeatInterval=10000000 --conf spark.port.maxRetries=100 --conf spark.yarn.executor.memoryOverhead=10240 /var/SP/data/home/csanc109/src/devel/use-cases/churn_nrt/src/projects/models/virgin/analysis/virgin_analysis.py > /var/SP/data/home/csanc109/logging/volumes_virgin_incrementals_top25k_clean_all_prev.log
...
if __name__ == "__main__":
    ...
    for closing_day in ["20200621", "20200630", "20200707", "20200714", "20200721"]:
        ...
        print("before set_label - count = {}".format(df_labels_join.count()))
        df_labels_join = set_label(spark, closing_day_B, df_labels_join, CHURN_WINDOW).na.fill({'label': 0.0})
        print("after set_label - count = {}".format(df_labels_join.count()))
        ...
        # cum_churn_rate = get_cumulative_churn_rate_fix_step(spark, df_labels_join, ord_col='scoring', label_col='label', step_=500)
        # df_pandas = cum_churn_rate.toPandas()
        ...
        # ----- removing previous msisdns in previous cycles
        to_remove_df = None
        first_cycle = move_date_n_cycles(closing_day_B, n=-4)
        for cycle_, df_prev in dfs_previous_dict.items():
            if cycle_ >= first_cycle:
                print("for closing_day_B={} - remove the msisdns for date={}".format(closing_day_B, cycle_))

                before_ = to_remove_df.count() if to_remove_df is not None else 0
                if to_remove_df is None:
                    to_remove_df = df_prev.select("msisdn")
                else:
                    to_remove_df = to_remove_df.join(df_prev.select("msisdn"), on=["msisdn"], how="outer").cache()
                print("AFTER dfs_previous before={} after={}".format(before_, to_remove_df.count()))

        if to_remove_df is not None:
            df_labels_join_clean = to_remove_df.select("msisdn").join(df_labels_join.select("msisdn", "label", "scoring"), ['msisdn'], 'right').where(to_remove_df['msisdn'].isNull())
            df_labels_join_clean = df_labels_join_clean.cache()
            vol_incremental_clean = df_labels_join_clean.count()
            print("vol_clean_A={} vol_clean_B={} volInc={}".format(to_remove_df.count(), df_labels_join.count(), vol_incremental_clean))
        else:
            print("Nothing previous to remove in date {}".format(closing_day_B))
            df_labels_join_clean = df_labels_join

        cum_churn_rate_clean = get_cumulative_churn_rate_fix_step(spark, df_labels_join_clean, ord_col='scoring', label_col='label', step_=2500)
        df_pandas_clean = cum_churn_rate_clean.toPandas()
        df_pandas_clean.to_csv("/var/SP/data/home/csanc109/data/virgin/df_inc_top{}_clean_all_prev_{}.csv".format(inc_top, closing_day_B), header=True, index=True)


        dfs_previous_dict[closing_day_B] = df_labels_join_clean
