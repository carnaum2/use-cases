...
if __name__ == "__main__":
    ...
    sc, spark, sql_context = get_spark_session("revamp_fbb")
    sc.setLogLevel('WARN')
    ...
    from churn_nrt.src.projects.models.revamp.model_classes import RevampFBB
    from churn_nrt.src.data_utils.Metadata import Metadata
    from churn_nrt.src.projects.models.revamp.metadata import get_metadata_fbb
    metadata_obj = Metadata(spark, get_metadata_fbb, ["num_cliente"], ["ids"])

    model_revamp = RevampFBB(spark, tr_date_, mode_, model_, 'asaezco', metadata_obj, segment, horizon)
    ...
    path_to_save = '/data/attributes/vf_es/revamp/validation_fbb/segment={}/year={}/month={}/day={}'.format(segment,int(tr_date_[:4]),int(tr_date_[4:6]),int(tr_date_[6:8]))

    df_val_preds.select('num_cliente', 'label', 'model_score').write.mode("overwrite").format("parquet").save(path_to_save)
    ...
    if mode_ == 'production':
        model_output_n = "revamp_fbb_" + segment
        model_revamp.insert_model_outputs(df_tt_preds.drop('msisdn', 'nif_cliente'), model_output_n, test_date=tt_date_, insert_top_k=None, extra_info_cols=[], day_to_insert=insert_day)  # insert the complete df
