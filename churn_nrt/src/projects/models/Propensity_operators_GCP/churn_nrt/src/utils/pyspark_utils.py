...
def _df_to_csv(df, cols=None, sep='\t'):
    if type(df) is DataFrame:
        ...
        collected = df
        if cols is None:
            cols = list(collected[0].asDict().keys())
    else:
    ...
    for r in collected:
        ...
        for c in cols:
            # print c, row_d[c]
            line.append(str(row_d[c]).encode("utf-8"))
        # print '-----'
...
def add_column_index (spark, df, offset=1, colName="rowId"):
    ...
    new_rdd = zipped_rdd.map(lambda row_rowId: ([row_rowId[1] +offset] + list(row_rowId[0])))
...
def count_nans(df):
    ...
    return {col_:val for col_,val in list(df_count_collect.asDict().items()) if val>0}
...
def extract_columns(cols_list, include="", exclude="", exclude_set=None):
    ...
    return [c for c in cols_list if (
            c not in exclude_set and (len(exclude) == 0 or exclude not in c) and re.match('.*' + include + '.*', c,
                                                                                          re.IGNORECASE))]
