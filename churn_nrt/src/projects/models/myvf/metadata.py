...
from churn_nrt.src.projects.models.myvf.constants import HIGH_CORRELATED_VARS_VERSION3, HIGH_CORRELATED_VARS_VERSION1_2, WEB_HIGH_CORRELATED_VARS_VERSION1_2, WEB_HIGH_CORRELATED_VARS_VERSION3
...
def get_metadata(spark, sources=None, filter_correlated_feats=False):
    ...
    if filter_correlated_feats and any([ss.startswith("myvf") and not ss.startswith("myvfweb") for ss in sources]):
    ...
    if filter_correlated_feats and any([ss.startswith("myvfweb") for ss in sources]):
        before_rem = metadata.count()
        if version == 3:
            print("[metadata] Removing high correlated feats from metadata - myvfweb using v3")
            metadata = metadata.where(~col("feature").isin(WEB_HIGH_CORRELATED_VARS_VERSION3))

        else:
            print("[metadata] Removing high correlated feats from metadata - myvfweb using v1_2")
            metadata = metadata.where(~col("feature").isin(WEB_HIGH_CORRELATED_VARS_VERSION1_2))
